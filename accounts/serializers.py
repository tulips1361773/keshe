...
class UserRegistrationSerializer(serializers.ModelSerializer):
    ...
    achievements = serializers.CharField(required=False, allow_blank=True, write_only=True,
                                         help_text='教练员比赛成绩描述')
    ...
    def validate(self, attrs):
        ...
        if attrs.get('user_type') in ['coach', 'student']:
            # 必须选择校区
            campus_id = attrs.get('campus_id')
            if not campus_id:
                raise serializers.ValidationError("必须选择所属校区")
